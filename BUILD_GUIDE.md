...
## 国内拉取 base 镜像失败时（EOF / 超时）

若出现 `failed to do request: Head "https://...mirrors.../library/python/manifests/3.11-slim": EOF`，多半是当前配置的 Docker 镜像站（如 USTC）不可用。可任选其一：

**方式 1：用国内镜像地址作为 base 再构建**

在项目根目录执行（DaoCloud 镜像）：

```bash
PYTHON_IMAGE=docker.m.daocloud.io/library/python:3.11-slim docker-compose up -d --build
```

若仍失败，可尝试关闭 Docker Desktop 的镜像加速：**Settings → Docker Engine**，删掉或注释 `registry-mirrors` 后 **Apply**，再执行：

```bash
docker-compose up -d --build
```

（直连 Docker Hub，可能较慢或需网络畅通。）

**方式 2：仅后端 + 沙箱用国内镜像，MariaDB 仍用默认**
...
- **ARG PYTHON_IMAGE**默认 `python:3.11-slim`；可在 docker-compose 里用 `build.args.PYTHON_IMAGE` 覆盖（如国内镜像）。
- **FROM ${PYTHON_IMAGE}**基于该 Python 镜像，得到一个 Linux + Python 环境。
- **RUN for f in ... sed ...**把系统里 apt 的源改成国内镜像（阿里云），方便后面 `apt-get` 能拉包。
- **RUN apt-get update && apt-get install ... curl ...**装 `ca-certificates`、`curl`，然后用 curl 下载 Docker 官方给的静态编译好的 **Docker CLI**（docker 命令），放到 `/usr/local/bin/docker`。这样容器里没有 Docker 守护进程，但可以通过挂载宿主机的 ` /var/run/docker.sock` 来执行 `docker run`（在宿主机上起沙箱容器）。
- **COPY requirements.txt .**把本地的 `backend/requirements.txt` 复制进镜像当前目录（/app）。
- **RUN pip install --no-cache-dir -r requirements.txt**在镜像里安装 requirements.txt 里列出的所有 Python 包（FastAPI、uvicorn、sqlalchemy、pymysql 等）。**这里用到的是 requirements.txt 这一份配置文件。**
- **COPY . .**把 `backend/` 下除 .dockerignore 排除外的所有文件复制进镜像的 /app（包括 main.py、models、services 等）。
- **EXPOSE 8000 / CMD ...**
...
- **ARG PYTHON_IMAGE / FROM ${PYTHON_IMAGE} AS builder**第一个阶段：基于同一 Python 镜像，当作「构建阶段」。
- **RUN pip install --no-cache-dir --target /install pandas numpy**把 pandas、numpy 装到目录 `/install`，不装进系统 site-packages。这样下一阶段只拷贝这一坨，镜像更小。
- **FROM ${PYTHON_IMAGE}**（第二阶段）再开一个干净的 Python 镜像，不要 builder 里的 gcc 等。
- **COPY --from=builder /install ...**把 builder 里 `/install` 下的包拷贝到当前镜像的 site-packages，所以最终镜像里只有 Python + pandas/numpy，没有编译工具。
- **COPY runner.py /app/runner.py**把沙箱入口脚本拷进去。
...
- **ARG NODE_IMAGE / FROM ${NODE_IMAGE}**基于 Node 镜像（默认 node:18-alpine），可用 docker-compose 的 `NODE_IMAGE` 覆盖。
- **COPY package*.json ./**把 `frontend/package.json`（和若有 package-lock.json）拷进镜像的 /app。**这里用到的就是 package.json 这一份配置文件。**
- **RUN npm install**在镜像里执行 npm install，根据 package.json 和 lock（若有）安装所有依赖（react、antd、axios、react-scripts 等）。**所以前端依赖完全由 package.json（及 lock）决定。**
- **COPY . .**再把前端源码（src、public 等）拷进去。
